...
message(STATUS "ALPHA - ABCDE")
  if (EXISTS "${CMAKE_MODULE_PATH}")
    message(STATUS "Module Path Enhanced - was already set")
...
#     set (CMAKE_MODULE_PATH ${CMAKE_INSTALL_PREFIX} ${CMAKE_INSTALL_PREFIX}/${APPEND_BUILD_TYPE} ${CMAKE_MODULE_PATH} ${CMAKE_MODULE_PATH}/${APPEND_BUILD_TYPE})
    ...
    message(STATUS "Module Path Enhanced - was not already set")
...
message(STATUS "ALPHA - FGHIJ")
#message(STATUS "VALUES - cmake_module_path: ${}")
message(STATUS "VALUES - cmake_prefix_path: ${CMAKE_PREFIX_PATH}")
message(STATUS "VALUES - cmake_install_prefix: ${CMAKE_INSTALL_PREFIX}")
...
if (OWN_FIND_ALL_PACKAGE)
	...
	find_package(${func_name_cap} COMPONENTS Fortran)
#	find_package(${func_name_cap} COMPONENTS Fortran HINTS ${CMAKE_PREFIX_PATH})
...
# Not sure why, but the HINTS makes this not work - I would have thought the reverse
          message(STATUS "${func_name_cap} Our find will use HINTS - CMAKE_MODULE_PATH - WHICH IS ${CMAKE_MODULE_PATH}")
#          find_package(${func_name_cap} MODULE HINTS ${CMAKE_MODULE_PATH})
#          find_package(${func_name_cap} HINTS ${CMAKE_INSTALL_PREFIX} ${CMAKE_PREFIX_PATH})
#	  find_package(${func_name_cap} PATHS /home/cfsd/laster/bmad/external)
...
##      COMMAND echo hello
#
# it seems to me there is nothing to copy from include/plplot
...
#  file(COPY ${CMAKE_ROLLOUT_CMAKE_FILES}/Find${func_name}lib.cmake  DESTINATION ${${func_name}_DESTDIR})
if (OWN_FINDPACKAGE)
  file(COPY ${CMAKE_ROLLOUT_CMAKE_FILES}/Find${func_name_cap}.cmake  DESTINATION ${${func_name_cap}_DESTDIR})
endif()
#        find_package(${func_name_cap} HINTS ${CMAKE_PREFIX_PATH})
        find_package(${func_name_cap})
        if(${func_name}_FOUND)
	  ...
	  message(STATUS "Could not find ${func_name_cap} - building ${func_name} now")
        ...
        message(STATUS "We are not really building openmpi, we must got here to check to see if we have it.")
#  	now_really_build_openmpi()
...
message(STATUS "JUST A TEST! - ${pre_func_name_cap} - HINTS - To LOOK IN ${CMAKE_MODULE_PATH} ${CMAKE_PREFIX_PATH}")
#  find_package(${pre_func_name_cap} HINTS ${CMAKE_MODULE_PATH} ${CMAKE_PREFIX_PATH})
#  find_package(${pre_func_name_cap} HINTS ${CMAKE_MODULE_PATH})
...
#   endif()
  ...
  elseif (${func_name} STREQUAL "openmpi")
    message(STATUS "Doing copy for openmpi")
    if (OWN_FINDPACKAGE)
      file(COPY ${CMAKE_ROLLOUT_CMAKE_FILES}/Find${func_name_cap}.cmake  DESTINATION ${${func_name}_DESTDIR})
   endif()
